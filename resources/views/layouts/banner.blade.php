@section('page-banner')
<section class="free-post">
  <div class="relative z-10 mx-auto w-max-full bg-gray-100 sm:h-[135px] md:h-[320px] lg:h-[400px] xl:h-[400px]">
    <img class="z-0 flex mx-auto w-auto h-auto bg-cover items-center" src="../bannerV2.png"alt="Banner" />
    <img class="flex mx-auto max-w-sm items-center md:h-auto h-20 md:-mt-64 -mt-24" src="../logo.PNG" alt="Logo" />
  </div>
  <div style="top: -12px" class="relative z-20 mx-auto max-w-6xl items-center p-2 text-sm sm:text-sm md:text-md lg:text-base xl:text-base shadow text-green-800 bg-green-50 border border-green-200" role="alert">
    <span class="font-medium">Akce!</span> <span class="text-sm sm:text-sm md:text-md lg:text-base xl:text-base">Při objednávce betonových plotů i s montáží akce 15%. Pouze na materiál 5%.
  </div>
</section>  
@endsection